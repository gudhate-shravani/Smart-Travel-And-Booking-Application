...
class _AICoGuideScreenState extends State<AICoGuideScreen> {
  ...
  String _extractTextFromResponse(dynamic json) {
    if (json == null) return '';
    // If response includes "candidates" -> try to find text there
    try {
    ...
    // 2) Common fallback: search entire JSON for the first long-ish string
    return _searchForString(json);
  }

  // Recursively search a structure for first non-empty string >= 10 chars
  ...
  // --- Call Gemini text model for a text query ---
  Future<String> _callGeminiForText(String prompt) async {
    // Make sure your API key is enabled and the Generative Language API is enabled in GCP
    final url = Uri.parse(
      // query param "key" works, or you can use Authorization header with Bearer token
    ...
    final requestBody = {
      "contents": [
        {
          // role + parts make prompts clearer to the server
          "role": "user",
          "parts": [
            {
              "text":
