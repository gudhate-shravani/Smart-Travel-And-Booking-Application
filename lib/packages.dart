...
// A single data model to represent a travel package.
class TravelPackage {
  final String title;
...
class _TravelPackagesScreenState extends State<TravelPackagesScreen> {
  // --- MOCK DATA ---
  final List<TravelPackage> _packages = [
    TravelPackage(
      title: 'Tokyo Cherry Blossom Experience',
      location: 'Tokyo, Japan',
      imageUrl: 'https://picsum.photos/seed/cherryblossom/800/600',
      rating: 4.9,
      reviewCount: 1240,
    ...
    TravelPackage(
      ...
      imageUrl: 'https://picsum.photos/seed/europe/800/600',
      rating: 4.8,
      reviewCount: 2150,
  ...
  Widget build(BuildContext context) {
    final featuredPackages = _packages.where((p) => p.isFeatured).toList();
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            _buildSectionHeader('All Packages', count: _packages.length),
            const SizedBox(height: 16),
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      title: const Text('Travel Packages', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
      centerTitle: true,
      actions: [
        IconButton(icon: const Icon(Icons.redeem, color: Colors.black87), onPressed: () {}),
      ],
    );
  }
  ...
  Widget _buildCategoryFilters() {
    final categories = ['All Packages', 'Cultural', 'Adventure', 'Relax'];
    final icons = [Icons.wb_sunny, Icons.account_balance, Icons.hiking, Icons.beach_access];
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Icon(icons[index], color: isSelected ? Colors.deepOrange.shade600 : Colors.grey.shade600, size: 20),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Flexible(child: Text(package.location, style: TextStyle(color: Colors.grey, fontSize: 12), overflow: TextOverflow.ellipsis)),
                        const SizedBox(width: 8),
                         Icon(Icons.timer_outlined, size: 14, color: Colors.grey),
                        Flexible(child: Text(package.duration, style: TextStyle(color: Colors.grey, fontSize: 12), overflow: TextOverflow.ellipsis)),
                      ],
                    ),
                     const SizedBox(height: 4),
                     Row(
                      children: [
                         ...
                         Container(
                          ...
                          child: Text(package.difficulty, style: TextStyle(color: Colors.green.shade800, fontWeight: FontWeight.bold, fontSize: 12))
                         ),
                      ],
                     ),
                    const Spacer(), // Use a Spacer to push the bottom row down
                    Row(
  ...
  Widget _buildAllPackagesList() {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        return _buildAllPackagesCard(_packages[index]);
      },
    );
  }
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(package.title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                   SizedBox(height: 4),
                   Row(children: [
                      Icon(Icons.location_on, size: 14, color: Colors.grey),
                      SizedBox(width: 4),
                      ...
                      Icon(Icons.calendar_today, size: 14, color: Colors.grey),
                       SizedBox(width: 4),
                      Flexible(child: Text(package.date, style: TextStyle(color: Colors.grey, fontSize: 12), overflow: TextOverflow.ellipsis,)),
                   ]),
                   SizedBox(height: 4),
                   Row(children: [
                      Icon(Icons.star, color: Colors.amber, size: 14),
                      SizedBox(width: 4),
                      Text('${package.rating} • ${package.groupSize}', style: TextStyle(color: Colors.grey, fontSize: 12)),
                   ]),
                   SizedBox(height: 8),
                   Row(
                    children: [
                        Text('\$${package.price}', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.deepOrange, fontSize: 16)),
                        SizedBox(width: 4),
                        Text('\$${package.originalPrice}', style: TextStyle(color: Colors.grey, decoration: TextDecoration.lineThrough, fontSize: 12)),
                        Spacer(),
                        Container(
                          padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                          decoration: BoxDecoration(
                          ...
                          child: Text(package.duration, style: TextStyle(color: Colors.grey.shade800, fontSize: 10, fontWeight: FontWeight.bold)),
                        )
                    ],
                   )
                ],
